...
public static class BookCsvImporter
{
    ...
    private static ParseOutcome ParseLine(string line)
    {
        ...
        {
            { Length: < 4 } =>
                new ParseFailed($"очікую щонайменше 4 колонки, отримав {parts.Length}"),

            [_, "", _, _, ..] or [_, _, "", _, ..] =>
                new ParseFailed("ISBN або назва порожні"),

            [_, _, _, var year, ..]
                when !int.TryParse(year, NumberStyles.Integer,
                    CultureInfo.InvariantCulture, out int y)
                    || y < 1450 || y > DateTime.Now.Year =>
                new ParseFailed(
                    $"рік '{year}' поза допустимими межами (1450..{DateTime.Now.Year})"),
